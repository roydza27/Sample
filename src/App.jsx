...
const loadAutoPushHistory = async () => {
  ...
  return (
    <div className="min-h-screen bg-gray-900">
          ...
          <div className="flex space-x-1 mb-6 bg-gray-800 p-1 rounded-lg border border-gray-700">
            ...
            <button onClick={() => setActiveTab('dashboard')}
              className={`flex-1 py-2 px-4 rounded-lg font-medium transition-colors ${
                activeTab==='dashboard'?'bg-primary-600 text-white':'text-gray-400 hover:bg-gray-700'
              }`}>
            ...
            <button onClick={() => setActiveTab('autopush')}
              className={`flex-1 py-2 px-4 rounded-lg font-medium transition-colors ${
                activeTab==='autopush'?'bg-primary-600 text-white':'text-gray-400 hover:bg-gray-700'
              }`}>
              <div className="flex items-center justify-center space-x-2">
                <Zap className="w-4 h-4" /><span>Auto-Push</span>
              </div>
            </button>

            <button onClick={() => setActiveTab('analytics')}
              className={`flex-1 py-2 px-4 rounded-lg font-medium transition-colors ${
                activeTab==='analytics'?'bg-primary-600 text-white':'text-gray-400 hover:bg-gray-700'
              }`}>
              <div className="flex items-center justify-center space-x-2">
      ...
      {showSettings && (
        <SettingsModal workspacePath={workspacePath} onClose={() => setShowSettings(false)} onWorkspaceChange={handleWorkspaceChange} onLog={addLog} />
      )}
    </div>
  );
}
